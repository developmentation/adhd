...
| Blue | Defensive architecture review, ASVS and CAS audit | 15 skill phases plus scanner reuse | Mostly probabilistic (LLM judgement) |
...
zero-finding fall-throughs when their ecosystem is absent (no `go.mod`,
no `package.json`, no Python source, and so on) so cross-stack runs
stay quiet.

| Check | What it does | Mode | External deps |
|-------|--------------|------|---------------|
| `api_base_url_audit` | Cross-checks the API base URL across frontend source, tests, CI env, env templates, and Go backend defaults. Flags any drift so dev, staging, and prod values stay aligned. | Deterministic | None |
...
| `dangerous_patterns_scan` | Pattern-based SAST for JS, TS, and Vue (`innerHTML=`, `eval(`, `document.write(`, `dangerouslySetInnerHTML`) and for Go (`fmt.Sprintf` with SQL fragments). | Deterministic | None |
...
| `license_scan` | Detects GPL, AGPL, and LGPL packages in npm dependency trees. Prefers `license-checker` and falls back to reading `node_modules/<pkg>/package.json`. | Deterministic | `license-checker` (optional, npm) |
...
| `osv_scan` | Dependency vulnerability scan via OSV.dev across Maven, Gradle, pip, Poetry, Pipenv, Cargo, RubyGems, Composer, NuGet, and npm. | Deterministic | Network access to api.osv.dev |
...
| `rule10_intensifier` | Detects vague intensifiers (very, really, quite, extremely, incredibly) in prose. | Deterministic | None |
...
| `rule12_ai_smell` | Detects five composite AI-prose tics: sycophantic openings ("Great question"), foreshadowing or recap ("As we will see"), over-explanation tells ("Let's dive in"), hedge-stacks (three or more of might, could, may in one sentence), and "as an AI" disclaimers. Regex-driven and deterministic despite the name. | Deterministic | None |
...
| `endpoint_discovery` | Endpoint and directory discovery. Combines `feroxbuster` (optional), `robots.txt`, `sitemap.xml`, and a bundled 80-path wordlist covering admin, API, health, debug, config, and source-control routes. | Deterministic | `feroxbuster` (optional) |
| `http_headers` | Audits HSTS, CSP, X-Content-Type-Options, X-Frame-Options, Referrer-Policy, and friends. Pure Node fetch, no binaries needed. | Deterministic | None (Node fetch) |
...
| `waf_detect` | WAF detection. Prefers `wafw00f` and falls back to a heuristic probe of three requests covering Cloudflare, Akamai, AWS WAF, Azure, Sucuri, and Incapsula signatures. | Deterministic | `wafw00f` (optional) |
...
| `13-requirements-map` | Maps every codified business, functional, and non-functional requirement onto the application map so audit gaps are visible. | Probabilistic | `@anthropic-ai/sdk` |
...
(semgrep, govulncheck, nmap, and friends) the harness still runs and
